...
enum EObjectFlags : uint32_t
{
	...
	// < Object will be marked as root set on construction and not be garbage collected, even if unreferenced ( DO NOT USE
	//     THIS FLAG in HasAnyFlags() etc
	// )
	RF_TagGarbageTemp = 0x00000100,

	// < This is a temp user flag for various utilities that need to use the garbage collector. The garbage collector itself
	...
	// < Flagged on UObjects that are used to create UClasses ( e.g. Blueprints) while they are regenerating their UClass on
	//     load ( See FLinkerLoad::CreateExport
	// (
	// )
	// )
	...
	// < Do not export object to text form ( e.g. copy/ paste). Generally used for sub-objects that can be regenerated from
	//     data in their parent object.
	RF_LoadCompleted = 0x00200000,

	// < Object has been completely serialized by linkerload at least once. DO NOT USE THIS FLAG, It should be replaced with
	//     RF_WasLoaded.
